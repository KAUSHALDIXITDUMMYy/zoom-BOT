...
export default function AdminDashboard() {
  ...
  const addExternalMeeting = async () => {
    ...
    setLoading(true)
    try {
      const assignedNames = subscribers
        .filter((s) => newMeeting.assignedSubscriberIds.includes(s.id))
        .map((s) => s.name)
            ...
            <Dialog open={isCreateMeetingOpen} onOpenChange={setIsCreateMeetingOpen}>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="subscriber">Assign to Subscribers</Label>
                    <div className="border rounded-md p-2 max-h-40 overflow-auto">
                      {subscribers.map((s) => {
                        const selected = newMeeting.assignedSubscriberIds.includes(s.id)
                        return (
                              ...
                              onChange={(e) => {
                                setNewMeeting((prev) => ({
                                  ...
                                  assignedSubscriberIds: e.target.checked
                                    ? [...prev.assignedSubscriberIds, s.id]
                                    : prev.assignedSubscriberIds.filter((id) => id !== s.id),
                                }))
                              }}
                            />
